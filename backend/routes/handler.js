...
router.get('/user-performance', (req, res) => {
    ...
    const qry = `
        SELECT up.performance_id, u.user_id, up.score
...
router.post('/update-performance/:userId', async(req, res) => {
    const userId = req.params.userId;

    // Query the database to count the number of badges for the user
    const badgeCountQuery = 'SELECT COUNT(*) as badgeCount FROM user_badges WHERE user_id = ?';
    const badgeCountResult = await pool.query(badgeCountQuery, [userId]);

    const newScore = badgeCountResult[0].badgeCount;

    // Update the user's score in the user_performance table
    const updateScoreQuery = 'INSERT INTO user_performance (user_id, score) VALUES (?, ?) ON DUPLICATE KEY UPDATE score = ?';
    pool.query(updateScoreQuery, [userId, newScore, newScore], (err, result) => {
        if(err) {
...
// Endpoint to get employee performance by organization
router.get('/employee-performance/organization/:organizationId', async (req, res) => {
        const organizationId = req.params.organizationId;
        const qry = `
            SELECT u.user_id, u.first_name, u.last_name, u.email, up.user_id, up.score
            FROM users u
            INNER JOIN user_performance up ON up.user_id = u.user_id
            WHERE u.organization_id = ? AND u.user_role != 'management';
        `;
